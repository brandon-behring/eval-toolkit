...
## What is an eval, and what does this toolkit do? {#what-is-an-eval}
...
## The EvalSlice concept {#eval-slice}
...
## Run `evaluate()` and read the output {#evaluate}
...
## (Optional) Validate the JSON {#validate}
...
## (Optional) Add a claim {#add-a-claim}
...
### `ValueError: labels must be in {0, 1}` {#error-labels}
...
### `KeyError: missing strata column 'X'` {#error-strata}
...
### Bootstrap CIs are very wide {#error-wide-ci}
...
### `RuntimeError: PR curve has no thresholds` {#error-pr-curve}
...
### `'TYPE_CHECKING' import error` for pandas {#error-pandas}
